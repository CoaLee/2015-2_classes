...
let check : M.exp -> M.typ = fun expr ->
  let rec checkWithEnv : (M.exp, typ_env) -> typ_scheme = fun (expr, env) ->
	  match expr with
		| CONST c -> let t = (match c with S _ -> TString | N _ -> TInt | B _ -> TBool) in SimpleTyp t
		| VAR x -> let rec resolv : typ_env -> M.id -> typ_scheme = fun envr var ->
		           ( match envr with
							   | [] -> raise (M.TypeError "Type Failed")
								 | (hid, hsch)::t -> if hid = var then hsch else resolv t var )
							 in resolv env x
		| FN (x, e) -> let 
		| APP (e1, e2) ->
		| LET (d, e) ->
